...
export default function SEOAlternativePage() {
      ...
      <div style={{ padding: '20px 80px 36px' }}>
        ...
        <div className="sx-card" style={{ padding: 20, marginTop: 22, display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 22, alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div className="sx-logo-tile" style={{ width: 56, height: 56, background: competitor.color[0], color: competitor.color[1], fontSize: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 12, border: '1px solid var(--border)' }}>{competitor.letter}</div>
            <div>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                      {Object.entries(a.diff).map(([k, v]) => (
                        <span key={k} className="sx-chip" style={{ fontSize: 10.5 }}>
                          ...
                          <span style={{ color: (v as string).startsWith('−') ? 'var(--pos)' : v === 'sim' || v === 'nativo' || v === 'melhor' ? 'var(--acc)' : 'var(--text-dim)' }}>{v as string}</span>
                        </span>
                      ))}
                    </div>
                  </div>
          ...
          <div style={{ position: 'sticky', top: 20 }}>
            <div className="sx-card" style={{ padding: 16, marginBottom: 14 }}>
              ...
              {[
                ...
                <div key={t as string} style={{ padding: '6px 0', fontSize: 12, color: a ? 'var(--text)' : 'var(--text-dim)', borderLeft: '2px solid ' + (a ? 'var(--acc)' : 'transparent'), paddingLeft: 10, fontWeight: a ? 600 : 400, cursor: 'pointer' }}>
                  {t as string}
                </div>
              ))}
            </div>
            ...
            <div className="sx-card" style={{ padding: 16 }}>
              ...
              {[
                ...
                <a key={n as string} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: i < 5 ? '1px solid var(--border)' : 0, fontSize: 12.5, color: 'var(--text)', cursor: 'pointer' }}>
                  <span>Alternativas a <strong>{n as string}</strong></span>
                  <span className="sx-mono" style={{ color: 'var(--text-muted)', fontSize: 10.5 }}>{c as number}k buscas/mês</span>
                </a>
              ))}
            </div>
          </div>
