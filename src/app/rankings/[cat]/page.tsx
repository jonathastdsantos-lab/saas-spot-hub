    ...
    <div className="sx-screen" style={{ overflow: 'auto' }}>
      ...
      <div style={{ padding: '24px 80px 0' }}>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
          Rankings / <span style={{ color: 'var(--text)', textTransform: 'capitalize' }}>{params.cat.replace('-', ' ')}</span>
        </div>
        <h1 style={{ fontSize: 36, fontWeight: 500, letterSpacing: '-0.025em', margin: '14px 0 4px', maxWidth: 720, lineHeight: 1.1 }}>
          Melhores da categoria <span className="sx-serif" style={{ fontStyle: 'italic', color: 'var(--acc)', fontWeight: 400, textTransform: 'capitalize' }}>{params.cat.replace('-', ' ')}</span>
        </h1>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {[
                ...
                <label key={t as string} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--text-dim)', cursor: 'pointer' }}>
                  <input type="checkbox" defaultChecked={i === 1 || i === 2} style={{ accentColor: 'var(--acc)' }} />
                  <span style={{ flex: 1 }}>{t as string}</span>
                  <span style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: 10.5 }}>{n as number}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {[['🇧🇷 BR', true], ['🇺🇸 EN'], ['🇪🇸 ES'], ['🇲🇽 MX'], ['🇦🇷 AR']].map(([t, a]) => (
                <span key={t as string} className="sx-chip" style={{
        ...
        <div>
          ...
          <div className="sx-card" style={{
            ...
            <div style={{ fontSize: 13, lineHeight: 1.55 }}>
              <strong>Resumo IA:</strong> Para negócios desta categoria com até 5 colaboradores, a recomendação é começar com o <span style={{ color: 'var(--acc)' }}>Fluxia</span> (R$ 49/mês) ou o <span style={{ color: 'var(--acc)' }}>SalonPro</span> (R$ 79/mês). <span style={{ color: 'var(--acc)' }}>ZapCRM</span> é melhor se WhatsApp é o único canal. <a style={{ color: 'var(--ai)', cursor: 'pointer' }}>Ver análise completa →</a>
            </div>
          </div>
                ...
                <div key={p.id + i} className="sx-card" style={{ padding: 16, display: 'grid', gridTemplateColumns: '48px 56px 1fr auto 120px', gap: 16, alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 14, marginTop: 6, fontSize: 11, color: 'var(--text-muted)' }}>
                      ...
                      <span className="sx-mono">{p.integrations || 10} integrações</span>
                      <span className="sx-mono"><span style={{ color: 'var(--pos)' }}>●</span> {p.uptime || '99.9%'}</span>
                    </div>
                  </div>
          ...
          <div className="sx-card" style={{ padding: 22, marginTop: 22 }}>
            <h3 style={{ fontSize: 18, fontWeight: 600, margin: '0 0 8px' }}>Como escolhemos as melhores ferramentas</h3>
            <p style={{ fontSize: 13, color: 'var(--text-dim)', lineHeight: 1.6, margin: '0 0 12px' }}>
              Analisamos 47 plataformas que atendem o segmento no Brasil. Nosso ranking considera 6 dimensões: SaaS Score™ (peso 30%), avaliações verificadas (25%), preço relativo ao porte do negócio (20%), recursos específicos (15%), suporte em PT-BR (5%) e velocidade de implementação (5%).
            </p>
            <div style={{ display: 'flex', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
              <span className="sx-chip">{params.cat}</span>
              <span className="sx-chip">SaaS</span>
