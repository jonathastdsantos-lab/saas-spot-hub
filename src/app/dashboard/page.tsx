      ...
      <div style={{ width: 220, borderRight: '1px solid var(--border)', background: 'var(--bg-2)', padding: '16px 12px', display: 'flex', flexDirection: 'column' }}>
        ...
        {[
          ...
          <button key={t as string} style={{
            ...
            <Icon name={icon as string} size={14} stroke={a ? 'var(--acc)' : 'var(--text-muted)'} />
            {t as string}
          </button>
        ))}
        ...
        {[
          ...
          <button key={t as string} style={{
            ...
            <Icon name={icon as string} size={14} stroke="var(--text-muted)" />
            {t as string}
          </button>
        ))}
            ...
            <div className="sx-card" style={{ padding: 18 }}>
              ...
              <svg width="100%" height="200" viewBox="0 0 700 200" preserveAspectRatio="none">
                ...
                {(() => {
                  ...
                  const max = 200;
                  const path = (d: number[], mode?: string) => {
                ...
                <div key={label as string} style={{ marginBottom: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11.5, marginBottom: 4 }}>
                    ...
                    <span style={{ fontFamily: 'var(--font-mono)' }}>
                      <strong style={{ color: 'var(--text)' }}>{(n as number).toLocaleString('pt-BR')}</strong>
                      <span style={{ color: 'var(--text-muted)', marginLeft: 6 }}>{(pct as number) < 1 ? (pct as number).toFixed(2) : pct}%</span>
                    </span>
                  </div>
                  <div style={{ height: 6, background: 'var(--surface-3)', borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{ width: `${100 - i * 18}%`, height: '100%', background: color as string, opacity: 1 - i * 0.1 }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginTop: 12 }}>
            ...
            <div className="sx-card" style={{ padding: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <a style={{ fontSize: 11, color: 'var(--text-muted)', cursor: 'pointer' }}>Ver todas</a>
              </div>
              {[
            ...
            <div className="sx-card" style={{ padding: 16 }}>
              ...
              {[
                ...
                <div key={t as string} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: i < 4 ? '1px solid var(--border)' : 0 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, fontWeight: 500 }}>{t as string}</div>
                    <div style={{ fontSize: 10.5, color: 'var(--pos)', fontFamily: 'var(--font-mono)', marginTop: 1 }}>{v as string}</div>
                  </div>
                  <span className="sx-mono" style={{ fontSize: 11, color: 'var(--text-dim)' }}>{(n as number).toLocaleString('pt-BR')}</span>
                </div>
              ))}
            </div>
