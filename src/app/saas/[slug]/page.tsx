  ...
  return (
    <div className="sx-screen" style={{ overflow: 'auto' }}>
      ...
      <div style={{ padding: '14px 80px 0', fontSize: 12, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
        Descobrir / {p.cat} / <span style={{ color: 'var(--text)' }}>{p.name}</span>
      </div>
        ...
        <div style={{ display: 'flex', gap: 18, alignItems: 'flex-start' }}>
          ...
          <div style={{ paddingTop: 4 }}>
            ...
            <div style={{ display: 'flex', gap: 18, alignItems: 'center', fontSize: 12, color: 'var(--text-dim)' }}>
              ...
              <span style={{ height: 14, width: 1, background: 'var(--border)' }} />
              <span className="sx-mono">{p.integrations || 15} integrações</span>
              <span style={{ height: 14, width: 1, background: 'var(--border)' }} />
              <span className="sx-mono"><span style={{ color: 'var(--pos)' }}>●</span> Uptime {p.uptime || '99.9%'}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2.4fr 1fr', gap: 12, height: 380 }}>
          <div className="sx-img-ph" style={{ position: 'relative', overflow: 'hidden' }}>
            <div style={{
              ...
              background: `linear-gradient(135deg, rgba(200,255,62,0.3), transparent 50%), repeating-linear-gradient(135deg, rgba(255,255,255,0.025) 0 8px, transparent 8px 16px), var(--surface-2)`,
            }} />
      ...
      <div style={{ padding: '0 80px 60px', display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24 }}>
        ...
        <div>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 600, margin: '8px 0 12px' }}>O que é o {p.name}?</h3>
          <p style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--text-dim)', margin: '0 0 24px' }}>
            {p.desc} Fundado em 2022 por ex-funcionários da Pipedrive Brasil, o {p.name} já atende 8.400 empresas — de salões a startups B2B — com foco obsessivo em automação de funil pelo WhatsApp. Sua proposta: substituir CRM + WhatsApp Business API + bot de qualificação por um único produto.
          </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="sx-card" style={{ padding: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
              ...
              <ScoreGauge score={p.score} size={64} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[
                ...
                <div key={k as string} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 11, color: 'var(--text-dim)', width: 92 }}>{k as string}</span>
                  ...
                  <span className="sx-mono" style={{ fontSize: 11, width: 24, textAlign: 'right' }}>{v as number}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="sx-card" style={{ padding: 16 }}>
            ...
            {[
              ['Fundada em', p.founded_year || '2022'],
              ['HQ', p.hq || 'São Paulo · BR'],
              ['Funcionários', p.employees_range || '24–50'],
              ['Suporte', '24/7 PT-BR'],
              ['SLA', p.uptime || '99.9%'],
              ['LGPD', 'Conforme'],
              ['Trial', p.trial_days ? `${p.trial_days} dias sem cartão` : '14 dias sem cartão'],
            ].map(([k, v]) => (
