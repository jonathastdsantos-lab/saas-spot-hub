    ...
    <div className="sx-screen">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', height: 'calc(100% - 49px)' }}>
        ...
        <aside style={{ borderRight: '1px solid var(--border)', padding: '28px 24px', background: 'var(--bg-2)' }}>
          ...
          {[
            { n: 1, t: 'Conta & verificação', s: 'Domínio confirmado', done: true },
            { n: 2, t: 'Identidade do produto', s: 'Nome, logo, descrição', done: true },
            { n: 3, t: 'Recursos & integrações', s: 'O que seu SaaS faz', active: true },
            { n: 4, t: 'Planos & preços', s: 'Modelo de cobrança' },
            { n: 5, t: 'Mídia', s: 'Screenshots, vídeo' },
            { n: 6, t: 'SEO & publicação', s: 'Revisar e publicar' },
          ].map(s => (
        ...
        <div style={{ overflow: 'auto', display: 'grid', gridTemplateColumns: '1fr 380px' }}>
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                {[
                  ...
                  <span key={t} className="sx-chip" style={{ background: 'var(--acc-soft)', color: 'var(--acc)', borderColor: 'transparent' }}>
                    {t}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 10, background: 'var(--ai-soft)', borderRadius: 8, fontSize: 11.5, color: 'var(--text-dim)' }}>
                ...
                IA detectou <strong style={{ color: 'var(--text)' }}>3 recursos</strong> que seu site menciona mas não estão na lista: <span style={{ color: 'var(--acc)' }}>RAG</span>, <span style={{ color: 'var(--acc)' }}>chamadas de voz</span>, <span style={{ color: 'var(--acc)' }}>cobrança recorrente</span>.
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
                {[
                  ...
                  <div key={i} className="sx-card" style={{ padding: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 26, height: 26, borderRadius: 6, background: c, color: '#fff', fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{l}</div>
                    <div style={{ fontSize: 11.5, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{n}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                {[
                  ...
                  <label key={t} className="sx-card" style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                    <div style={{
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {[['Português', true], ['English', true], ['Español', true], ['Français', false], ['Italiano', false], ['Deutsch', false]].map(([l, on]) => (
                  <button key={l} className="sx-chip" style={{
          ...
          <div style={{ padding: '32px 28px', borderLeft: '1px solid var(--border)', background: 'var(--bg-2)' }}>
            ...
            <div className="sx-card" style={{ padding: 0, overflow: 'hidden' }}>
              <div style={{ height: 80, background: 'linear-gradient(135deg, #C8FF3E40, transparent), repeating-linear-gradient(135deg, rgba(255,255,255,0.03) 0 8px, transparent 8px 16px), var(--surface-2)' }} />
            ...
            {[
              ...
              <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', fontSize: 12 }}>
                ...
                <span style={{ color: on ? 'var(--text-dim)' : 'var(--text)' }}>{t}</span>
              </div>
            ))}
