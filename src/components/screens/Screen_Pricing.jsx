// Tela 14: Pricing dos anunciantes
            ...
            <div key={plan.name} className="sx-card" style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
                {plan.features.map(([f, on]) => (
                  <div key={f} style={{ display: 'flex', gap: 8, fontSize: 11.5, color: on ? 'var(--text-dim)' : 'var(--text-muted)' }}>
                    {on ? (
                    ...
                    <span style={{ lineHeight: 1.4, opacity: on ? 1 : 0.55 }}>{f}</span>
                  </div>
                ))}
              </div>
            </div>
...
window.Screen_Pricing = Screen_Pricing;
